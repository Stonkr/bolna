...
def get_plivo_client():
    return plivo.RestClient(os.getenv("PLIVO_AUTH_ID"), os.getenv("PLIVO_AUTH_TOKEN"))


@retry(
    wait=wait_fixed(2),
    stop=stop_after_attempt(3),
    retry=retry_if_exception_type(plivo.exceptions.PlivoRestError),
)
async def create_plivo_call(call_details, telephony_host, bolna_host, call_id):
    ...
    plivo_client.calls.create(
        ...
        answer_url=f"{telephony_host}/plivo_connect?bolna_host={bolna_host}&agent_id={call_details.get('agent_id')}&to_phone_number={call_details.get('recipient_phone_number')}&call_id={encoded_call_id}",
    ...
    try:
        ...
        if context_data:
            if "recipient_data" not in context_data:
                raise HTTPException(
                    status_code=400, detail="recipient_data not provided in context_data"
                )
        ...
        # encoded_call_id = urllib.parse.quote(call_id)
        call = await create_plivo_call(call_details, telephony_host, bolna_host, call_id)
...
async def plivo_connect(
    request: Request,
    bolna_host: str = Query(...),
    agent_id: str = Query(...),
    to_phone_number: str = Query(...),
    call_id: str = Query(...),
):
    try:
